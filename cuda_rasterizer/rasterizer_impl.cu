...
__global__ void duplicateWithKeys(int P, const float2 *points_xy,
  ...
  if (radii[idx] > 0) {
    ...
    getRectSpherical(points_xy[idx], radii[idx], rect_min, rect_max, grid);
    for (int y = rect_min.y; y < rect_max.y; y++) {
      for (int x = rect_min.x; x < rect_max.x; x++) {
        int x_index = x;
        int2 grid_pos = {((x_index % (int)grid.x) + (int)grid.x) % (int)grid.x,
                         y};
        uint64_t key = grid_pos.y * grid.x + grid_pos.x;
        key <<= 32;
...
CudaRasterizer::GeometryState::fromChunk(char *&chunk, size_t P) {
  ...
  // Instead of allocating P * 9 elements, allocate P * 12 for the 4rows by
  // 3cols M matrix
  obtain(chunk, geom.transMat, P * 12, 128);
...
int CudaRasterizer::Rasterizer::forward(
    ...
    std::function<char *(size_t)> imageBuffer, const int P, const int width,
  ...
  // Retrive total number of Gaussians instances to launch and resize aux
  // buffers
  int num_rendered;
  ...
  const float *transMat_ptr =
      transMat_precomp != nullptr ? transMat_precomp : geomState.transMat;

  CHECK_CUDA(FORWARD::render(tile_grid, block, imgState.ranges,
                             binningState.point_list, width, height, projmatrix,
                             geomState.means2D, transMat_ptr, geomState.depths,
                             geomState.normal_opacity, imgState.accum_alpha,
                             imgState.n_contrib, out_others),
             debug)
