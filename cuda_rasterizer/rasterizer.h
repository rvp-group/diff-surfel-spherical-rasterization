...
public:
  ...
  static int forward(std::function<char *(size_t)> geometryBuffer,
                     ...
                     const float *projmatrix, const float tan_fovx,
                     float tan_fovy, const bool prefiltered, float *out_others,
  ...
  static void backward(const int P, int R, const int width, int height,
                       ...
                       char *binning_buffer, char *image_buffer,
                       const float *dL_depths, float *dL_dmean2D,
                       float *dL_dnormal, float *dL_dopacity, float *dL_dmean3D,
                       float *dL_dtransMat, float *dL_dscale, float *dL_drot,
                       bool debug);
};
} // namespace CudaRasterizer
